  ...
  {#each getFilteredTodos() as todo, i}
    <div class="flex bg-[#222231] rounded-lg p-2 group {todo.done ? 'opacity-50' : ''}">
      <div class="flex items-center pl-2">
        ...
        hover:!opacity-100 hover:bg-[#179f43]
        checked:bg-[#44634E]
      ...
      <div class="flex">
        <div class="flex items-center ">
          <button onclick={() => deleteTodo(todo.id)} class="px-3 cursor-pointer
          opacity-0
          group-hover:opacity-50 hover:!opacity-100">
            <img src={x} alt="Delete icon">
          </button>
        </div>
        <div class="cursor-grab flex items-center pr-3 opacity-0
          group-hover:opacity-50 hover:!opacity-100 ">
